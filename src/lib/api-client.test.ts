...
describe("api client", () => {
  ...
  it("resolves with undefined on a 204 No Content response instead of parsing a body", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response(null, { status: 204 })),
    );

    await expect(apiRequest(`/api/v1/archive/entry-1`, { method: "DELETE" })).resolves.toBeUndefined();
  });
});
